...
d3.csv("data/tx_county_recent.csv").then(data=> {
  ...
  gCounty.append("rect")
    .attr("width", xScaleC(Number(data[102]["Total Doses Allocated"].replace(/,/g, ''))))
    .attr("height", BARHEIGHT)
    .attr("fill", "#fa9441")
    .attr("transform", "translate(0, 200)")
  gCounty.append("text")
    .attr("x", xScaleC(Number(data[102]["Total Doses Allocated"].replace(/,/g, ''))))
    .attr("y", 195)
    .attr("fill-opacity", "50%")
    .text(`Allocated: ${data[102]["Total Doses Allocated"]}`)

  gCounty.append("rect")
    .attr("width", xScaleC(Number(data[102]["Vaccine Doses Administered"].replace(/,/g, ''))))
    .attr("height", BARHEIGHT)
    .attr("fill", "#97d4ea")
    .attr("transform", "translate(0, 200)")
  gCounty.append("text")
    .attr("x", xScaleC(Number(data[102]["Vaccine Doses Administered"].replace(/,/g, ''))))
    .attr("y", 255)
    .text(`Administered: ${data[102]["Vaccine Doses Administered"]}`)

  gCounty.append("rect")
    .attr("width", xScaleC(Number(data[102]["People Vaccinated with at least One Dose"].replace(/,/g, ''))))
    .attr("height", BARHEIGHT)
    .attr("fill", "#00bde3")
    .attr("transform", "translate(0, 200)")
  gCounty.append("text")
    .attr("x", xScaleC(Number(data[102]["People Vaccinated with at least One Dose"].replace(/,/g, ''))))
    .attr("y", 275)
    .text(`One Dose: ${data[102]["People Vaccinated with at least One Dose"]}`)
  gCounty.append("line")
    .attr("x1", xScaleC(Number(data[102]["People Vaccinated with at least One Dose"].replace(/,/g, ''))))
    .attr("y1", 240)
    .attr("x2", xScaleC(Number(data[102]["People Vaccinated with at least One Dose"].replace(/,/g, ''))))
    .attr("y2", 265)
    .attr("stroke", "#00bde3")
    .attr("stroke-dasharray", "3 1")

  gCounty.append("rect")
    .attr("width", xScaleC(Number(data[102]["People Fully Vaccinated"].replace(/,/g, ''))))
    .attr("height", BARHEIGHT)
    .attr("fill", "#07648d")
    .attr("transform", "translate(0, 200)")
  gCounty.append("text")
    .attr("x", xScaleC(Number(data[102]["People Fully Vaccinated"].replace(/,/g, ''))))
    .attr("y", 295)
    .text(`Both Doses: ${data[102]["People Fully Vaccinated"]}`)
  gCounty.append("line")
    .attr("x1", xScaleC(Number(data[102]["People Fully Vaccinated"].replace(/,/g, ''))))
    .attr("y1", 240)
    .attr("x2", xScaleC(Number(data[102]["People Fully Vaccinated"].replace(/,/g, ''))))
    .attr("y2", 280)
    .attr("stroke", "#07648d")
    .attr("stroke-width", 2)
